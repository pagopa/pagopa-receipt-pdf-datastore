...
locals {
  env_secrets = {
    ...
    "RECEIPTS_STORAGE_CONN_STRING_BASE64" : base64encode(data.azurerm_storage_account.receipts_sa.primary_connection_string),
    "RECEIPTS_COSMOS_CONN_STRING_BASE64" : base64encode("AccountEndpoint=https://pagopa-${var.env_short}-${local.location_short}-${local.domain}-ds-cosmos-account.documents.azure.com:443/;AccountKey=${data.azurerm_cosmosdb_account.receipts_cosmos.primary_key};"),
    "BIZEVENTS_COSMOS_CONN_STRING_BASE64" : base64encode("AccountEndpoint=https://pagopa-${var.env_short}-${local.location_short}-bizevents-ds-cosmos-account.documents.azure.com:443/;AccountKey=${data.azurerm_cosmosdb_account.bizevents_cosmos.primary_key};")
  }
  env_variables = {
